...
export function isExportFormatV1(obj: any): obj is AgentXExportFormatV1 {
...
export function cleanData(data: any): LatestExportFormat {
  ...
  {
    // Attempt to convert to AgentX format
    return {
      app: 'AgentX',
      version: 1,
...
export const exportData = async (database: Database, user: User) => {
  ...
  link.download = `AgentX_history_${currentDate()}.json`;
  link.href = url;
